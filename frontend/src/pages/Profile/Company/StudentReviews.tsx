...
interface CompanyReviewsProps {
  user_id: number;
  /** อนุญาตให้กดไลค์ไหม (ถ้าไม่ส่ง จะอิงจาก roleId === 3 อัตโนมัติ) */
  allowLike?: boolean;
  /** อนุญาตให้สร้างรีวิวไหม (ยังไม่ใช้ใน UI นี้ แต่เตรียมไว้) */
...
const CompanyReviews: React.FC<CompanyReviewsProps> = ({
  ...
  // autoplay เฉพาะเมื่อมีหลายสไลด์
  useEffect(() => {
  ...
  // โหลดรายการรีวิวที่กดไลค์ไว้ (เฉพาะคนที่มีสิทธิ์กดไลค์ + มี user id)
  useEffect(() => {
    const fetchLiked = async () => {
      ...
      try {
        ...
        // เงียบๆ ไม่ต้องรบกวนผู้ใช้
      }
    };
    fetchLiked();
  }, [canLike]);

  // หยุด/เริ่ม autoplay เมื่อ hover
  useEffect(() => {
  ...
  const handleLike = async (reviewId: number) => {
    if (!canLike) {
      message.info("เฉพาะนักศึกษาจึงจะกดถูกใจรีวิวได้");
      return;
    }
    ...
    <div className="reviews-section">
      ...
      <div className="reviews-overview">
        ...
        <div className="rating-summary">
          <div className="avg-rating">{averageRating.toFixed(2)}</div>
          <div className="stars">
            <Rate allowHalf disabled value={roundToHalf(averageRating)} />
          </div>
          <div className="rating-count">{allReviews.length} รีวิว</div>
        </div>
      ...
      <div className="review-filters">
        {[
          { key: "all", label: `ทั้งหมด (${allReviews.length})`, icon: <FaTh /> },
          { key: "5", label: "5 ดาว", icon: <FaStar /> },
          { key: "4", label: "4 ดาว", icon: <FaStar /> },
          { key: "3", label: "3 ดาว", icon: <FaStar /> },
          { key: "2", label: "2 ดาว", icon: <FaStar /> },
          { key: "1", label: "1 ดาว", icon: <FaStar /> },
          { key: "recent", label: "ล่าสุด", icon: <FaClock /> },
        ].map((btn) => (
          <button
            key={btn.key}
            className={`filter-btn ${currentFilter === btn.key ? "active" : ""}`}
      ...
      <div className="reviews-carousel-wrapper">
        ...
        {/* Content */}
        <div className="reviews-carousel" style={{ transform: `translateX(${translateX}px)` }}>
            ...
            >
              <div className="review-content">
                ...
                <div className="review-header">
                  ...
                  <div className="review-date">{dayjs(review.date).fromNow()}</div>
                </div>

                {/* Rating */}
                <div className="review-rating">{starsDisplay(review.rating)}</div>
              ...
              <div className="review-footer">
                <button
                  ...
                  title={!canLike ? "เฉพาะนักศึกษาจึงจะกดถูกใจรีวิวได้" : undefined}
                >
                  <FaHeart />
                </button>
              </div>
      ...
      <div className="review-counter-horizontal">
        {filteredReviews.length > 0 ? (
          <>
            ...
            {Math.min((currentSlide + 1) * reviewsPerSlide, filteredReviews.length)}{" "}
