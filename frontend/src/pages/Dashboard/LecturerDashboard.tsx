...
import {
  getAcademicOverview, GetAcademicStaffByUserIdForNewCompany, getAcademicTrend,
  listAcademicApplications, listAcademicStudents,
} from "@/services/https";
...
const PRESETS: { label: string; value: PresetValue }[] = [
  { label: "7 วัน", value: "7d" }, { label: "30 วัน", value: "30d" },
  { label: "90 วัน", value: "90d" }, { label: "กำหนดเอง", value: "custom" },
];
...
const convertToCSV = (data: Record<string, any>[]) => {
  ...
  const headers = Object.keys(data[0]), esc = (val: unknown) => (val == null ? "" : /[",\n]/.test(String(val)) ? `"${String(val).replace(/"/g, '""')}"` : String(val));
...
const exportToCSV = (data: Record<string, any>[] = [], filename: string) => {
  ...
  a.href = url; a.setAttribute("download", `${filename}.csv`); document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
};
...
const STATUS_STROKES: Record<string, string> = { ผ่าน: "#22c55e", กำลังพิจารณา: "#3b82f6", นัดสัมภาษณ์แล้ว: "#6366f1", รอการนัดสัมภาษณ์: "#f59e0b", ไม่ผ่าน: "#ef4444", ไม่ได้รับเลือก: "#dc2626" };
const FAIL_LABEL = "ไม่ผ่าน/ไม่ได้รับเลือก";
const fmtDate = (d?: string) => (d ? new Date(d).toLocaleDateString("th-TH") : "-");
const fmtDateTime = (d?: string) => (d ? new Date(d).toLocaleString("th-TH", { dateStyle: "medium", timeStyle: "short" }) : "-");
...
const AcademicDashboard = () => {
  ...
  // เดิมใช้ useState แต่ setter ไม่ได้ใช้ → เปลี่ยนเป็นค่าคงที่
  const page = 1;
  const pageSize = 10;
  const q = "";
  const status = "";
  ...
  const [loadingStudentApps] = useState(false);

  const fetchAll = useCallback(async () => {
    setLoading(true);
    if (!userID) { messageApi.error("ไม่พบ user id ใน localStorage"); return; }
    const staffRes = await GetAcademicStaffByUserIdForNewCompany(userID);
    if (!staffRes) { navigate("/lecturer/add-lecturer", { replace: true }); messageApi.info("โปรดเพิ่มข้อมูลอาจารย์ก่อนใช้งานแดชบอร์ด"); return; }
    try {
  ...
  const fetchDailyTrend = useCallback(async () => {
    if (!userID) return;
    let start: string | undefined, end: string | undefined, days: number | undefined;
    if (preset !== "custom") days = preset === "7d" ? 7 : preset === "30d" ? 30 : 90;
    else if (range) { start = range[0].format("YYYY-MM-DD"); end = range[1].format("YYYY-MM-DD"); }
    else days = 30;
    try {
      const data = await getAcademicTrend(userID, { start, end, days });
      const normalized = (Array.isArray(data) ? data : []).map((p: any) => ({
        date: p.date, total: Number(p.total || 0), pass: Number(p.pass || 0), review: Number(p.review || 0),
        interviewed: Number(p.interviewed || 0), waiting_schedule: Number((p as any).waiting_schedule ?? (p as any).waiting ?? 0), fail: Number(p.fail || 0),
      }));
      setDailyTrend(normalized);
    } catch (e) { console.error(e); messageApi.error("โหลดกราฟรายวันไม่สำเร็จ"); }
  }, [userID, preset, range, messageApi]);

  useEffect(() => { fetchAll(); }, [fetchAll]);
  useEffect(() => { fetchDailyTrend(); }, [fetchDailyTrend]);

  const dailyChartData = useMemo(() => dailyTrend.map((p: any) => ({
    name: p.date, total: Number(p.total || 0), ผ่าน: Number(p.pass || 0), กำลังพิจารณา: Number(p.review || 0),
    นัดสัมภาษณ์แล้ว: Number(p.interviewed || 0), รอการนัดสัมภาษณ์: Number(p.waiting_schedule || p.waiting || 0), [FAIL_LABEL]: Number(p.fail || 0),
  })), [dailyTrend]);

  const companiesCoop = useMemo(() => {
    const map = new Map<string, { company_name: string; applicants: any[]; last_apply_at?: string }>();
    toArray(apps).forEach((a: any) => {
      const cname = a.company_name || "ไม่ระบุบริษัท";
      if (!map.has(cname)) map.set(cname, { company_name: cname, applicants: [], last_apply_at: undefined });
      const bucket = map.get(cname)!; bucket.applicants.push(a);
      const t = a.updated_at || a.submit_at;
      if (t && (!bucket.last_apply_at || new Date(t) > new Date(bucket.last_apply_at))) bucket.last_apply_at = t;
    });
    return Array.from(map.values())
      .map(v => ({ key: v.company_name, company_name: v.company_name, applicants_count: v.applicants.length, last_apply_at: v.last_apply_at }))
      .sort((a, b) => b.applicants_count - a.applicants_count);
  }, [apps]);

  const latest5UniqueAdvisees = useMemo(() => {
    const sorted = [...toArray(apps)].sort(
      (a: any, b: any) => new Date(b.updated_at || b.submit_at || 0).getTime() - new Date(a.updated_at || a.submit_at || 0).getTime()
    );
    const seen = new Map<number | string, any>();
    for (const a of sorted) { const key = (a.student_id ?? a.student_full_name) as number | string; if (!seen.has(key)) seen.set(key, a); if (seen.size >= 5) break; }
    return Array.from(seen.values()).map((a: any) => ({
      student_id: a.student_id, name: a.student_full_name, status: a.status,
      updated_at: a.updated_at || a.submit_at, company_name: a.company_name, post_name: a.post_name,
    }));
  }, [apps]);
  ...
  const applicationsColumns = [
    ...
    { title: "ตำแหน่ง", dataIndex: "post_name", key: "post_name", ellipsis: true, render: (t: string) => <Tooltip title={t}>{t}</Tooltip> },
    { title: "สถานะ", dataIndex: "status", key: "status", width: 160, render: (s: string) => <Tag color={getStatusColor(s)} style={{ borderRadius: 8, fontWeight: 500 }}>{s}</Tag> },
    { title: "สมัครเมื่อ", dataIndex: "submit_at", key: "submit_at", width: 140, render: (d: string) => fmtDate(d) },
  ];
  ...
  return (
    <ConfigProvider
      theme={{
        token: {
          fontWeightStrong: 600, colorPrimary: "#1677ff", colorInfo: "#1677ff", colorSuccess: "#22c55e",
          colorWarning: "#f59e0b", colorError: "#ef4444", colorText: "#0f172a", colorTextSecondary: "#475569",
          colorBorder: "#e2e8f0", colorBgLayout: "#ffffff", colorBgContainer: "#ffffff",
          borderRadiusLG: 20, borderRadius: 12, fontSize: 14,
          fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, 'Noto Sans Thai', 'Prompt', sans-serif",
          lineHeight: 1.6,
        ...
        components: {
          Card: { headerBg: "transparent", paddingLG: 24, boxShadow: "0 1px 3px 0 rgba(0,0,0,0.06), 0 1px 2px 0 rgba(0,0,0,0.04)" },
          Table: { headerBg: "#f1f5f9", headerColor: "#1f2937", cellPaddingBlockSM: 12, rowHoverBg: "#f8fafc", borderColor: "#e2e8f0" },
          Button: { controlHeight: 40, borderRadius: 10, fontWeight: 500 },
          Segmented: { itemSelectedBg: "rgba(22, 119, 255, 0.12)", borderRadius: 10 },
          Modal: { borderRadiusLG: 16 }, Tag: { borderRadius: 8 },
        },
      }}
    >
      <Layout>
        ...
        <div className="dashboard-container">
          ...
          <Spin spinning={loading}>
              ...
              <Col xs={24} sm={8} lg={8}>
                <div className="kpi-card" style={{ animationDelay: ".02s" }}>
                  <div className="kpi-content">
                    ...
                    <div style={{ flex: 1 }}>
                      <Statistic
                        title={<span style={{ color: "#475569", fontSize: 14, fontWeight: 600 }}>จำนวนนักศึกษา</span>}
              ...
              <Col xs={24} sm={8} lg={8}>
                <div className="kpi-card" style={{ animationDelay: ".08s" }}>
                  <div className="kpi-content">
                    ...
                    <div style={{ flex: 1 }}>
                      <Statistic
                        title={<span style={{ color: "#475569", fontSize: 14, fontWeight: 600 }}>ใบสมัครทั้งหมด</span>}
                    ...
                    companiesCoop.slice(0, 5).map((c: any, i: number) => (
                      <div key={c.key} className="list-item">
                        <div className="list-item-header">
                          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            <div
                              style={{
                                ...
                                background: `linear-gradient(135deg, ${["#1677ff","#0ea5e9","#22c55e","#f59e0b","#3b82f6"][i%5]}, ${["#1d4ed8","#0284c7","#16a34a","#d97706","#1e40af"][i%5]})`,
                          ...
                          <Badge count={c.applicants_count} showZero style={{ backgroundColor: "#1677ff", boxShadow: "0 2px 4px rgba(22,119,255,.3)" }} />
                        </div>
                        <div className="list-item-meta"><Text type="secondary">อัปเดตล่าสุด: {c.last_apply_at ? fmtDateTime(c.last_apply_at) : "-"}</Text></div>
                      </div>
                    ))
                  ) : (
                    <div style={{ padding: 40 }}><Empty description="ยังไม่มีข้อมูลบริษัท" /></div>
                  )}
                </Card>
              </Col>
                      ...
                      {latest5UniqueAdvisees.map((r: any) => (
                        <div key={`${r.student_id}-${r.updated_at}`} className="list-item">
                          ...
                          <div className="list-item-meta">
                            ...
                            <Text type="secondary">อัปเดตล่าสุด: {r.updated_at ? fmtDateTime(r.updated_at) : "-"}</Text>
                          </div>
                        </div>
                      ))}
                    </Row>
                  ) : (
                    <div style={{ padding: 40 }}><Empty description="ยังไม่มีการอัปเดตสำหรับนักศึกษาที่อยู่ในการดูแล" /></div>
                  )}
                </Card>
              </Col>
            ...
            <Card
              ...
              title={
                <Space>
                  <div style={{ width: 32, height: 32, borderRadius: 8, background: "linear-gradient(135deg, #3b82f6, #1d4ed8)", display: "flex", alignItems: "center", justifyContent: "center", color: "white" }}>
                    <TrendingUpIcon />
                  </div>
              ...
              extra={
                <Space size={12} wrap>
                  <Segmented<PresetValue>
                    ...
                    value={preset}
                    onChange={(val) => { setPreset(val as PresetValue); if (val !== "custom") setRange(null); }}
                    style={{ borderRadius: 8 }}
                  />
                  {preset === "custom" && (
                    <RangePicker value={range ?? undefined} onChange={(vals) => setRange(vals ? [vals[0]!, vals[1]!] : null)} format="YYYY-MM-DD" allowClear style={{ borderRadius: 8 }} />
                  )}
                  <Tooltip title="ส่งออก (CSV)">
                    <Button className="secondary-button" icon={<ExportOutlined />} onClick={() => exportToCSV(dailyChartData as AnyRow[], `แนวโน้มรายวัน_${dayjs().format("YYYYMMDD")}`)} />
                  </Tooltip>
                </Space>
              }
              bodyStyle={{ padding: 0 }}
          ...
          <Drawer
            ...
            title={
              <Space>
                <div style={{ width: 28, height: 28, borderRadius: 6, background: "linear-gradient(135deg, #1677ff, #1d4ed8)", display: "flex", alignItems: "center", justifyContent: "center", color: "white" }}>
                  <FileTextOutlined />
                </div>
            ...
            extra={
              <Space wrap>
                <Text type="secondary" style={{ fontWeight: 600 }}>สถานะ:</Text>
                <Select allowClear placeholder="ทั้งหมด" style={{ width: 220 }} value={appsDrawerStatus || undefined} onChange={(v) => setAppsDrawerStatus(v || "")}>
                  {statuses.map((s) => (<Option key={s} value={s}>{s}</Option>))}
                </Select>
                <Button className="secondary-button" onClick={() => exportToCSV((appsDrawerStatus ? apps.filter((a) => a.status === appsDrawerStatus) : apps) ?? [], "ใบสมัครทั้งหมด")}>ส่งออก CSV</Button>
              </Space>
            }
          ...
          >
            <div style={{ padding: 24 }}>
              <Table
                ...
                scroll={{ x: true, y: 520 }}
                sticky
                dataSource={(appsDrawerStatus ? apps.filter((a) => a.status === appsDrawerStatus) : apps) ?? []}
                columns={applicationsColumns}
                rowKey="id"
                pagination={{ pageSize: 12, showTotal: (t, r) => `${r[0]}-${r[1]} จาก ${t} รายการ`, showSizeChanger: true, showQuickJumper: true }}
                locale={{ emptyText: <Empty description="ยังไม่มีข้อมูลใบสมัคร" /> }}
              />
            </div>
          </Drawer>
          ...
          <Modal
            ...
            title={<span style={{ fontSize: 16, fontWeight: 700 }}>ใบสมัครของ <span className="gradient-text">{selectedStudentInfo ? `${selectedStudentInfo.first_name} ${selectedStudentInfo.last_name}` : "นักศึกษา"}</span></span>}
            width={900}
            onCancel={() => { setStudentApplicationsModal(false); setSelectedStudentApps([]); setSelectedStudentInfo(null); }}
            footer={[
              <Button key="export" className="action-button" onClick={() => {
          ...
          >
            <Spin spinning={loadingStudentApps}>
              <Table
                ...
                scroll={{ x: true, y: 420 }}
                sticky
                dataSource={Array.isArray(selectedStudentApps) ? selectedStudentApps : []}
                rowKey="id"
                pagination={{ pageSize: 8 }}
                locale={{ emptyText: <Empty description="ยังไม่มีข้อมูลใบสมัครของนักศึกษาคนนี้" /> }}
                columns={[
                  { title: "บริษัท", key: "company", ellipsis: true, render: (_: any, r: any) => <Tooltip title={r.company_name || r.company || "-"}>{r.company_name || r.company || "-"}</Tooltip> },
                  { title: "ตำแหน่ง", key: "position", ellipsis: true, render: (_: any, r: any) => <Tooltip title={r.position || r.post_name || "-"}>{r.position || r.post_name || "-"}</Tooltip> },
                  { title: "สถานะ", dataIndex: "status", key: "status", width: 140, render: (s: string) => <Tag color={getStatusColor(s)} style={{ borderRadius: 8, fontWeight: 600 }}>{s || "-"}</Tag> },
